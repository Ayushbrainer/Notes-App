...
class updatePage extends StatefulWidget {
  Note currentNote;
  updatePage({this.currentNote});
  @override
  _updatePageState createState() => _updatePageState(currentNote: currentNote);
}

class _updatePageState extends State<updatePage> {
  Note currentNote;
  _updatePageState({this.currentNote});
        ...
        builder:(context) => FloatingActionButton(
          onPressed: (){
            if(title.text=="" && note.text==""){
              final snackbar = SnackBar(content: Text("Please write something to update"),backgroundColor: Colors.yellow,duration: Duration(seconds: 2),);
              ScaffoldMessenger.of(context).showSnackBar(snackbar);
            }else{
              postClass post = postClass();
              var code = post.updateData(Note(
                  title: title.text,
                  note: note.text,
              ...
              //   ScaffoldMessenger.of(context).showSnackBar(snackbar);
              // }else{
              //   final snackbar = SnackBar(content: Text("Note Saved succesfully ${code}"),backgroundColor: Colors.red,duration: Duration(seconds: 2),);
              //   ScaffoldMessenger.of(context).showSnackBar(snackbar);
              // }
            }
